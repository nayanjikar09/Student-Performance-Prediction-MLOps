    ...
    def initiate_model_trainer(self, train_array, test_array):
        try:
            ...
            if best_model_score < 0.6:
                logging.error(f"Best model score ({best_model_score}) is below threshold 0.6")
                raise CustomException("No best model found with score > 0.6")

            logging.info(f"Best found model: {best_model_name} with R² score: {best_model_score:.4f}")
            ...
            logging.info(f"R² score on test set: {r2_square:.4f}")
